...
// For some reason, the above types differ from those returned by getTypes, so
// this structure will cache the types generated by getTypes
var getTypesCache = {};
...
function getArguments(type) {
  ...
  return (0, _lodash.reduce)(fields, function (args, field) {
    ...
    if (field.type instanceof _type2.GraphQLScalarType || field.type instanceof _type2.GraphQLList && field.type.ofType instanceof _type2.GraphQLScalarType || field.type instanceof _type2.GraphQLEnumType || field.type instanceof _type2.GraphQLList && field.type.ofType instanceof _type2.GraphQLEnumType) {
      args[field.name] = field;
      (0, _lodash.forEach)(['GT', 'GTE', 'LT', 'LTE', 'NE'], function (operator) {
        args[field.name + '_' + operator] = field;
      });
      args[field.name + '_ISNULL'] = {
        name: field.name,
        type: _type2.GraphQLBoolean
      };
    }

    if (field.type instanceof _type2.GraphQLObjectType) {
      args[field.name] = {
        name: field.name,
        type: new _type2.GraphQLList(_type2.GraphQLID)
      };
    }
...
function getType(graffitiModels, _ref2) {
  ...
  var rootType = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : null;
  ...
  var graphQLTypeFields = (0, _lodash.reduce)(fields, function (graphQLFields, _ref3, key) {
    var name = _ref3.name,
        ...
        nonNull = _ref3.nonNull,
        hidden = _ref3.hidden,
        hooks = _ref3.hooks,
        subfields = _ref3.fields,
        embeddedModel = _ref3.embeddedModel,
        enumValues = _ref3.enumValues;
    ...
    if (type === 'Array') {
      if (subtype === 'Object') {
        ...
        var graphQLSubType = enumValues && subtype === 'String' ? listToGraphQLEnumType(enumValues, getTypeFieldName(graphQLType.name, name + 'Enum')) : stringToGraphQLType(subtype);
        graphQLField.type = new _type2.GraphQLList(graphQLSubType);
        if (reference) {
...
function getTypes(graffitiModels) {
  var rebuildCache = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;

  if (!rebuildCache) {
    // TODO: iterate through full type tree and check if each type has been defined
  ...
  (0, _keys2.default)(types).forEach(function (typeName) {
